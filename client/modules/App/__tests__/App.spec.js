...
test('renders properly', t => {
  ...
  t.is(wrapper.find('Header').prop('toggleAddCompany'), wrapper.instance().toggleAddCompanySection);
...
test('calling toggleAddCompanySection dispatches toggleAddCompany', t => {
  const wrapper = shallow(
    <App {...props} />
  );

  wrapper.instance().toggleAddCompanySection();
  t.truthy(dispatch.calledOnce);
  t.truthy(dispatch.calledWith(toggleAddCompany()));
});
